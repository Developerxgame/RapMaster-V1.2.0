...
const randomTitles=[ 
'Money Dreams','Street Life','Rise Up','Hustle Hard','Golden Days','Midnight Vibes','City Lights','No Limits','Top Floor','Real Talk','Grinding Daily','Success Story','Never Stop','Diamond Heart','Legendary','From Zero','Big Dreams','Victory Lap','Unstoppable','Crown Me','Fire Flow','Boss Mode','Next Level','Pure Energy','Street King','Money Moves','Grind Time','Top Shelf','Real One','Empire State' 
];

const albumNames=[ 
'The Come Up','Street Dreams','Golden Hour','No Sleep','Legendary','From The Bottom','City Lights','The Journey','Rise & Grind','Victory Lap','Midnight Sessions','Crown Collection','Street Symphony','Diamond Life','Empire Rising','Pure Fire','Next Chapter','Boss Level','Money Talk','Street Royalty' 
];
...
className={`flex-shrink-0 flex items-center justify-center space-x-2 py-3 px-4 rounded-game transition-all text-sm ${activeTab===tab.id ? 'bg-neon-cyan text-white shadow-glow' : 'text-text-muted hover:text-text-primary hover:bg-dark-surface/30'}`} 
> 
...
className={`w-full p-3 bg-dark-surface/60 border border-dark-border text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:border-neon-cyan/50 rounded-game ${trackTitle && isDuplicateTitle(trackTitle) ? 'border-neon-red/50 focus:ring-neon-red/50' : 'focus:ring-neon-cyan/50'}`} 
maxLength={30} 
/> 
{trackTitle && isDuplicateTitle(trackTitle) && ( 
<p className="text-sm text-neon-red">⚠️ A track with this title already exists</p> 
)} 
<button onClick={generateRandomTitle} 
...
return ( 
<button key={lyrics.id} 
onClick={()=> canAfford && setSelectedLyrics(lyrics)} 
className={`w-full p-3 rounded-game text-left transition-all ${isSelected ? 'bg-neon-cyan text-white shadow-glow' : canAfford ? 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary' : 'bg-dark-surface/30 opacity-50 cursor-not-allowed text-text-muted'}`} 
disabled={!canAfford} 
> 
...
<button onClick={(e)=> {e.stopPropagation();toggleLyricsName();}} 
...
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < lyrics.quality ? isSelected ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
<div className="text-right"> 
<div className={`font-bold text-sm ${lyrics.price===0 ? 'text-neon-green' : isSelected ? 'text-white' : 'text-neon-cyan'}`}> 
...
return ( 
<button key={producer.id} 
onClick={()=> canAfford && setSelectedProducer(producer)} 
className={`w-full p-3 rounded-game text-left transition-all ${isSelected ? 'bg-neon-purple text-white shadow-glow' : canAfford ? 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary' : 'bg-dark-surface/30 opacity-50 cursor-not-allowed text-text-muted'}`} 
disabled={!canAfford} 
> 
<div className="flex justify-between items-center"> 
<div> 
...
<div className="flex items-center space-x-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < producer.quality ? isSelected ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
<div className="text-right"> 
<div className={`font-bold text-sm ${producer.price===0 ? 'text-neon-green' : isSelected ? 'text-white' : 'text-neon-cyan'}`}> 
...
return ( 
<button key={director.id} 
onClick={()=> canAfford && setSelectedDirector(director)} 
className={`w-full p-3 rounded-game text-left transition-all ${isSelected ? 'bg-neon-pink text-white shadow-glow' : canAfford ? 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary' : 'bg-dark-surface/30 opacity-50 cursor-not-allowed text-text-muted'}`} 
disabled={!canAfford} 
> 
<div className="flex justify-between items-center"> 
<div> 
...
<div className="flex items-center space-x-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < director.quality ? isSelected ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
<div className="text-right"> 
<div className={`font-bold text-sm ${director.price===0 ? 'text-neon-green' : isSelected ? 'text-white' : 'text-neon-cyan'}`}> 
...
className={`w-full p-3 rounded-game text-left transition-all ${isSelected ? 'bg-neon-green text-white shadow-glow' : canAfford ? 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary' : 'bg-dark-surface/30 opacity-50 cursor-not-allowed text-text-muted'}`} 
disabled={!canAfford} 
> 
<div className="flex justify-between items-center"> 
<div> 
...
<div className="flex items-center space-x-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < studio.quality ? isSelected ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
<div className="text-right"> 
<div className={`font-bold text-sm ${studio.price===0 ? 'text-neon-green' : isSelected ? 'text-white' : 'text-neon-cyan'}`}> 
...
<span className={`${(selectedLyrics?.price || 0) + (selectedProducer?.price || 0) + (selectedDirector?.price || 0) + (selectedStudio?.price || 0)===0 ? 'text-neon-green' : 'text-neon-cyan'}`}> 
{formatPrice( 
...
{/* Create Button */} 
<button onClick={createTrack} 
disabled={!canCreateTrack()} 
className={`w-full py-4 px-6 rounded-game font-bold transition-all text-sm ${canCreateTrack() ? 'game-button hover-glow' : 'bg-dark-surface/50 text-text-disabled cursor-not-allowed'}`} 
> 
...
className={`w-full p-3 bg-dark-surface/60 border border-dark-border text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:border-neon-cyan/50 rounded-game ${albumTitle && isDuplicateTitle(albumTitle,'album') ? 'border-neon-red/50 focus:ring-neon-red/50' : 'focus:ring-neon-cyan/50'}`} 
maxLength={30} 
/> 
...
<button onClick={generateRandomAlbumTitle} 
className="w-full p-3 bg-dark-surface/60 rounded-game hover:bg-dark-surface/80 transition-colors flex items-center justify-center space-x-2" 
> 
<SafeIcon icon={FiRefreshCw} className="text-text-muted" /> 
<span className="font-medium text-text-muted text-sm">Generate Random Title</span> 
...
className={`w-full p-3 rounded-game text-left transition-all ${selectedTracks.includes(track.id) ? 'bg-neon-cyan text-white shadow-glow' : 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary'}`} 
> 
<div className="flex justify-between items-center"> 
<div> 
...
<div className="flex items-center space-x-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < track.quality ? selectedTracks.includes(track.id) ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
</button> 
...
className={`w-full mt-4 py-4 px-6 rounded-game font-bold transition-all text-sm ${canCreateAlbum() ? 'game-button hover-glow' : 'bg-dark-surface/50 text-text-disabled cursor-not-allowed'}`} 
> 
<div className="flex items-center justify-center space-x-2"> 
...
className={`w-full p-3 rounded-game text-left transition-all ${selectedTrack===track.id ? 'bg-neon-red text-white shadow-glow' : 'bg-dark-surface/60 hover:bg-dark-surface/80 text-text-primary'}`} 
> 
<div className="flex justify-between items-center"> 
<div> 
<span className="font-medium text-sm">{track.title}</span> 
...
<div className="flex items-center space-x-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < track.quality ? selectedTrack===track.id ? 'text-white' : 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
</div> 
</button> 
...
<span className="text-text-primary">Total Cost</span> 
<span className={`${(selectedVideoProducer?.price || 0) + (selectedVideoDirector?.price || 0) + (selectedVideoStudio?.price || 0)===0 ? 'text-neon-green' : 'text-neon-cyan'}`}> 
{formatPrice( 
...
<button onClick={createMusicVideo} 
disabled={!canCreateVideo()} 
className={`w-full mt-4 py-4 px-6 rounded-game font-bold transition-all text-sm ${canCreateVideo() ? 'game-button hover-glow' : 'bg-dark-surface/50 text-text-disabled cursor-not-allowed'}`} 
> 
<div className="flex items-center justify-center space-x-2"> 
...
<div className="text-right"> 
<div className="text-base font-bold text-neon-cyan animate-neon">{release.views.toLocaleString()} views</div> 
<div className="text-xs text-neon-green">${Math.floor(release.earnings)} earned</div> 
</div> 
</div> 
<button onClick={()=> announceRelease(release.id)} 
...
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < track.quality ? 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
...
<button onClick={()=> releaseContent(track)} 
className="w-full game-button hover-glow text-sm" 
> 
...
<div className="flex items-center space-x-1 mb-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < album.quality ? 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
...
{!album.released ? ( 
<button onClick={()=> releaseContent(album)} 
className="w-full game-button hover-glow text-sm" 
> 
<SafeIcon icon={FiUpload} className="inline mr-2" /> 
Release on Rapify 
...
<div className="flex items-center space-x-1 mb-1"> 
{[...Array(5)].map((_,i)=> ( 
<SafeIcon key={i} icon={FiStar} className={`text-xs ${i < video.quality ? 'text-neon-orange' : 'text-text-disabled'}`} /> 
))} 
</div> 
...
{!video.released ? ( 
<button onClick={()=> releaseContent(video)} 
className="w-full game-button hover-glow text-sm" 
> 
<SafeIcon icon={FiUpload} className="inline mr-2" /> 
Release on RapTube 
